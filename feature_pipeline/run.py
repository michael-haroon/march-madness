...
from feature_pipeline.feature_importance  import (
    run_all_importance, synthetic_validation, build_rf, feat_imp_sfi,
    PurgedYearKFold,
)
from feature_pipeline.model               import (
...
def main():
    ...
    if not ff_2026.empty:
        ...
        # Log which rank_features are present / missing / all-NaN for 2026
        missing_2026  = [c for c in rank_features if c not in ff_2026.columns]
        all_nan_2026  = [c for c in rank_features if c in ff_2026.columns and ff_2026[c].isna().all()]
        present_2026  = [c for c in rank_features if c in ff_2026.columns and not ff_2026[c].isna().all()]
        print(f"  rank_features: {len(rank_features)} total")
        print(f"  present & non-NaN: {len(present_2026)}")
        if missing_2026:
            print(f"  MISSING from 2026 data ({len(missing_2026)}): {missing_2026}")
        if all_nan_2026:
            print(f"  ALL-NaN in 2026 ({len(all_nan_2026)}): {all_nan_2026}")

        # 2026 feature values per team
        snapshot_cols = [c for c in rank_features if c in ff_2026.columns]
        print("\n  2026 team feature snapshot:")
        snap = ff_2026.set_index("team")[snapshot_cols].T
