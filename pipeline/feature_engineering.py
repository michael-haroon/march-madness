...
from pipeline.config import MISSING_STRATEGY
...
def build_features(df: pd.DataFrame,
    ...
    Args:
        ...
        run_reconcile:        Drop ts_* columns that are highly correlated (r>0.95)
                              with their Kaggle equivalents.
...
KAGGLE_FEATURES = [
    ...
    "kg_stl_pg", "kg_blk_pg", "kg_scoring_margin", "kg_recent_margin",
...
def reconcile_cross_source(df: pd.DataFrame,
    ...
    compute Pearson r and recommend which to keep.

    If r > corr_threshold: they measure the same thing → keep Kaggle version
    (longer history + game-level granularity).
    If r < corr_threshold: they differ → investigate, keep both.

    Returns:
        df_clean: DataFrame with recommended redundant columns dropped
        report:   dict of {(kg_col, ts_col): {'r': float, 'action': str}}
    """
    # Candidate pairs: same underlying stat from both sources
    candidate_pairs = [
        ("kg_fg_pct",        "ts_fg_pct"),
    ...
    for kg_col, ts_col in candidate_pairs:
        ...
        if len(both) < 30:
            report[(kg_col, ts_col)] = {"r": np.nan, "n": len(both), "action": "too_few_rows"}
            continue

        r = both[kg_col].corr(both[ts_col])
        if r >= corr_threshold:
            action = "drop_ts"
            cols_to_drop.append(ts_col)
        else:
            action = "keep_both"
        report[(kg_col, ts_col)] = {"r": round(r, 3), "n": len(both), "action": action}

        if verbose:
            marker = "→ drop ts" if action == "drop_ts" else "→ keep both (diverge)"
            print(f"  {kg_col:28s} vs {ts_col:28s}  r={r:.3f}  {marker}")
...
def audit_feature_redundancy(df: pd.DataFrame,
    ...
    """
    from pipeline.feature_importance import onc_cluster
