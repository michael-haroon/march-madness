...
from pipeline.config import RF_PARAMS
...
def feat_imp_mdi(fit: BaggingClassifier,
    ...
    Returns DataFrame with mean and std (CLT normalised) for each feature.
    ...
    result = pd.concat({
        "mean": imp_df.mean(),
        "std":  imp_df.std() * imp_df.shape[0] ** -0.5,  # CLT SE
    }, axis=1)
    result /= result["mean"].sum()              # normalise to sum to 1
...
def feat_imp_mda(clf,
                 ...
                 scoring: str = "log_loss") -> pd.DataFrame:
    """
    ...
    for col in X.columns:
        ...
        imp  = base_scores - perm          # positive = shuffling this hurt performance
...
def feat_imp_cfi_mdi(fit: BaggingClassifier,
    ...
    mdi = feat_imp_mdi(fit, feat_names)
    records = {}
...
def synthetic_validation(n_samples: int = 500,
    ...
    mdi_result = feat_imp_mdi(clf, feat_names)
    sfi_result = feat_imp_sfi(
