...
def load_kalshi_trades(data_dir: str) -> pd.DataFrame:
    ...
    for endpoint in endpoints:
        ...
        for pf in parquet_files:
            ...
            from pipeline.data_loader import normalise_team
...
def compute_market_features(trades: pd.DataFrame,
                            cutoff_daynum: int = 151) -> pd.DataFrame:
    """
    ...
    Note: We do NOT apply a hard cutoff by date since we don't know the exact
    Final Four date per year. All trades are used. For the championship prediction,
    the Kalshi data through March 30, 2026 (DayNum ~147) is already pre-Final Four.
