...
def load_tournament_labels(kaggle_dir: str,
    ...
    """
    from pipeline.name_resolver import build_id_lookup, resolve_team_name
...
def load_kaggle_game_stats(kaggle_dir: str,
    ...
    def _agg_season(grp: pd.DataFrame) -> pd.Series:
        ...
        # Recent form: last 10 games by DayNum
        recent = grp.nlargest(10, "DayNum")
        ...
        return pd.Series({
            ...
            "kg_recent_margin": recent["Margin"].mean() if len(recent) > 0 else np.nan,
...
def load_team_stats(data_dir: str,
    ...
    """
    from pipeline.name_resolver import build_id_lookup, resolve_team_id, strip_conference
...
def load_all(data_dir: str = "data",
    ...
    if include_kaggle and os.path.isdir(kaggle_dir):
        ...
        if not game_stats.empty and "TeamID" in df.columns:
            ...
            from pipeline.name_resolver import build_id_lookup, resolve_team_id
        ...
        if not massey.empty and "TeamID" in df.columns:
            ...
            # Team sheets cover 2005-2026; Massey covers 2003-2026 for POM/RPI/SAG.
    ...
    if include_market:
        try:
            from pipeline.market_features import load_kalshi_trades, compute_market_features
            if verbose:
